...
###########################################################
# Set display options
template = paste('Al2O3: %{x:.2f}%\n', 'MgO: %{y:.2f}%\n', 
                 'CaO: %{z:.2f}%\n', 'SiO2: %{text:.2f}%')

axx = list(title = "Al2O3 %")
axy = list(title = "MgO %")
axz = list(title = "CaO %")
...
# Plot figure
dat %>% plot_ly() %>%
  add_trace(x = ~Al2O3, y = ~MgO, z = ~CaO, type="scatter3d", mode="markers",
            marker = list(color = ~Al2O3, size = 3, colorbar=list(title='SiO2 %'),
                          colorscale='Viridis'), 
            text = dat$SiO2,hoverinfo="x+y+z+text", hovertemplate = template)%>%
...
dat %>% plot_ly() %>%
  add_trace(x = ~Al2O3, y = ~MgO, z = ~CaO, type="scatter3d", mode="markers",
            marker = list(color = ~Tliq.C, size = 3, colorbar=list(title='Tliq ˚C'), colorscale='Jet'), 
            text = dat$SiO2, hoverinfo="x+y+z+text", hovertemplate = template)%>%
  layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz, aspectmode='cube'), 
...
dat %>% plot_ly() %>%
    ...
    add_trace(x = ~datt[,4], y = ~datt[,3], z = ~datt[,2], type="scatter3d", mode="markers",
...
axx = list(nticks = 10,range = c(0,100), title = "Al2O3 %")
...
dat %>% plot_ly() %>%
  ...
  add_trace(x = ~datt[,4], y = ~datt[,3], z = ~datt[,2], type="scatter3d", mode="markers",
            ...
            text = datt[,1], hoverinfo="x+y+z+text", hovertemplate = template, showlegend = FALSE)%>% 
  layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz, aspectmode = 'cube'),
